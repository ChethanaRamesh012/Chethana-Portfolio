      ...
      <div>
        {PROJECTS.map((project, index) => (
          <div key={index} className="mb-8 flex flex-wrap lg:flex-nowrap lg:justify-center">
            {/* Image Section */}
            <motion.div whileInView={{opacity:1, x:0}} initial={{ opacity:0, x:-100 }} transition={{ duration:0.5 }}  className="w-full lg:w-1/4 flex justify-center lg:justify-start mb-4 lg:mb-0 mr-20"> {/* Added margin-right here */}
              <img
                src={project.image}
                alt={project.title}
            ...
            <motion.div whileInView={{opacity:1, x:0}} initial={{ opacity:0, x:100 }} transition={{ duration:0.5 }}  className="w-full max-w-xl lg:w-3/4">
              ...
              {/* Project Link Section - Styled Separately Below Technologies */}
              {project.link && (
                <div className="mt-4">
                  <a
                    href={project.link}
                    ...
                    className="inline-block px-4 py-2 rounded-lg  text-purple-400 font-semibold"
                  >
                    🔗 View Project
                  </a>
                </div>
